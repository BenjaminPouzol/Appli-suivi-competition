...
describe('CompetitionFormulaire', () => {
  ...
  function remplir(): void {
    ...
    composant.formulaire.univers().value.set('football');
  ...
  describe('en creation', () => {
    ...
    it('envoie un POST puis revient a la liste', async () => {
      ...
      requete.flush(requete.request.body, { status: 201, statusText: 'Created' });
      await attendre();
  ...
  describe('en modification', () => {
    ...
    it("pre-remplit le formulaire et fige l'identifiant", () => {
    ...
    it("envoie un PUT sans l'identifiant dans le corps", async () => {
      ...
      expect(requete.request.body.nom).toBe('LoL');
      expect(requete.request.body.id).toBeUndefined();
